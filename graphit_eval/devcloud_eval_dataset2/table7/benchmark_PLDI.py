...
graphit_PATH = "./bin/"
if socket.gethostname() == "BlackBox":
  DATA_PATH = ""
  raise RuntimeError("ERROR: no hostname match.")
elif socket.gethostname() == "spiralcyh-10-90-80-51.spiral.local":
  DATA_PATH = "/sharedstorage/markb1/GAP_data/gap_format/"
else:
  raise RuntimeError("ERROR: no hostname match.")
#DATA_PATH = "../../../data/"
OUTPUT_DIR= "./outputs/"
...
def setVariables():
  if socket.gethostname() == "BlackBox":
    ...
    os.environ["KMP_AFFINITY"] = "verbose,explicit,proclist=[0-11]"
    os.environ["GOMP_CPU_AFFINITY"] = "0-11"
  elif socket.gethostname() == "spiralcyh-10-90-80-51.spiral.local":
...
def get_cmd_graphit(g, p, point):
    ...
    if p == "pi" or p == "sssp" or p == "sssp_delta_stepping" or p == "sssp_delta_stepping_lazy":
            ...
            for generic_app_name in args.applications:
                ...
                for point in points:
                  ...
                  if output:
                    ...
                    log_file.write("\n------------------------------------------------\n")
                    if generic_app_name in ["pi", "sir", "pr", "cc", "prd", "cf", "tc"]:
